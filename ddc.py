...
data_folder = 'office31perturbed'
batch_size = 32
n_class = 31
domain_src, domain_tar = 'webcam', 'dslr'
...
def train(dataloaders, model, optimizer):
    ...
    for e in range(n_epoch):
        stop += 1
        train_loss_clf, train_loss_transfer, train_loss_kl_src, train_loss_kl_tar, train_loss_total = 0, 0, 0, 0, 0
        model.train()
        for (src, tar) in zip(source_loader, target_train_loader):
            ...
            optimizer.zero_grad()
            label_source_pred, transfer_loss, mu_input_src, log_var_input_src, mu_bottleneck_src, log_var_bottleneck_src, mu_input_tar, log_var_input_tar, mu_bottleneck_tar, log_var_bottleneck_tar = model(data_source, data_target, return_gaussian_params=True) # Domain loss
            clf_loss = criterion(label_source_pred, label_source) # Source classification loss
            kl_loss_input_to_bottleneck_src = calculate_kl_divergence(mu_input_src, log_var_input_src, mu_bottleneck_src, log_var_bottleneck_src) # KL Divergence loss for source
            kl_loss_input_to_bottleneck_tar = calculate_kl_divergence(mu_input_tar, log_var_input_tar, mu_bottleneck_tar, log_var_bottleneck_tar) # KL Divergence loss for target
            ...
            train_loss_kl_src = kl_loss_input_to_bottleneck_src.detach().item() + train_loss_kl_src
            train_loss_kl_tar = kl_loss_input_to_bottleneck_tar.detach().item() + train_loss_kl_tar
            train_loss_total = loss.detach().item() + train_loss_total
        
        acc = test(model, target_test_loader)
        print(f'Epoch: [{e:2d}/{n_epoch}], cls_loss: {train_loss_clf/n_batch:.4f}, transfer_loss: {train_loss_transfer/n_batch:.4f}, kl_loss_src: {train_loss_kl_src/n_batch:.4f}, kl_loss_tar: {train_loss_kl_tar/n_batch:.4f}, total_Loss: {train_loss_total/n_batch:.4f}, acc: {acc:.4f}')
        if best_acc < acc:
            best_acc = acc
            torch.save(model.state_dict(), 'trained_original.pth')
            stop = 0
        if stop >= early_stop:
            break
