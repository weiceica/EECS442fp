...
class TransferNet(nn.Module):
    ...
    def forward(self, source, target=None, return_fc_features=False, return_gaussian_params=False):
        ...
        mu_input_src = self.fc_mu_input(source_features)
        log_var_input_src = self.fc_log_var_input(source_features)
        mu_bottleneck_src = self.fc_mu_bottleneck(source_bottleneck)
        log_var_bottleneck_src = self.fc_log_var_bottleneck(source_bottleneck)

        mu_input_tar, log_var_input_tar, mu_bottleneck_tar, log_var_bottleneck_tar = None, None, None, None
        if target is not None:
            mu_input_tar = self.fc_mu_input(target_features)
            log_var_input_tar = self.fc_log_var_input(target_features)
            mu_bottleneck_tar = self.fc_mu_bottleneck(target_bottleneck)
            log_var_bottleneck_tar = self.fc_log_var_bottleneck(target_bottleneck)

        if return_fc_features:
            return (source_features, source_bottleneck, source_clf, target_bottleneck)
        if return_gaussian_params:
            return (source_clf, transfer_loss, mu_input_src, log_var_input_src, mu_bottleneck_src, log_var_bottleneck_src, mu_input_tar, log_var_input_tar, mu_bottleneck_tar, log_var_bottleneck_tar)
